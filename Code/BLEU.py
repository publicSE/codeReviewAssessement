...
if __name__ == '__main__':
    ...
    for index in range(0,1291):
        ...
        reference_file = []
        with open("path2groundtruth"+str(index)+".txt", 'r') as f:
            for row in f:
                reference_file.append(row.strip())


        predictions = []
        with open("path2generatedReview"+str(index)+".txt", 'r') as f:
            for row in f:
                predictions.append(row.strip())
        (goldMap, predictionMap) = computeMaps(predictions, reference_file)
        print(bleuFromMaps(goldMap, predictionMap)[0])
        with open('./blue-issre-1291.txt', 'a') as f:
            f.write(str(bleuFromMaps(goldMap, predictionMap)[0]) + "\n")
